...
class HolidaysImport(orm.TransientModel):
    ...
    def _get_default_holidays(self, cr, uid, context=None):
        ...
        date_utc_from = get_utc_datetime(
            date_from, local_tz).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        ...
        if not holidays:
            raise osv.except_osv(_('Information'), _(
                'No holidays for the current timesheet.'))
    ...
    def import_holidays(self, cr, uid, ids, context=None):
        ...
        for holiday in wizard.holidays_ids:
            ...
            if holiday.date_from < timesheet.date_from:
                dt_ts_from = get_utc_start_of_day(
                    timesheet.date_from, local_tz)
            ...
            for day in range(nb_days):
                ...
                if not existing_ts_ids:
                    ...
                    product_id = al_ts_obj._getEmployeeProduct(
                        cr, uid, context)
                    journal_id = al_ts_obj._getAnalyticJournal(
                        cr, uid, context)
