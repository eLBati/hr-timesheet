...
class wizard_calendar_report(orm.TransientModel):
    ...
    def on_change_month(self, cr, uid, id, str_month, year):
        ...
        if year and str_month:
            ...
            res = {
                'value': {'to_date': to_date, 'from_date': date(year, month, 1).strftime('%Y-%m-%d')}}
        return res
                    ...
                    if reference_calendar.attendance_ids:
                        ...
                        for calendar_attendance in reference.attendance_ids:
                            if ((
                                ...
                                datetime.strptime(
                                    calendar_attendance.date_from, '%Y-%m-%d')
                                <= current_date
                            )):
                                ...
                                current_total_due = attendance_pool.time_sum(
                                    current_total_due,
                            ...
                            if current_overtime:
                                if current_overtime <= overtime_type.limit or not overtime_type.limit:
                                    ...
                                    totals_by_employee[employee_id]['total_types'][
                                        overtime_type.name] = attendance_pool.time_sum(
                                        totals_by_employee[
                                            employee_id]['total_types']
                                        [overtime_type.name], overtime_type.limit)
                                    current_overtime = attendance_pool.time_difference(
                                        overtime_type.limit,
                                        current_overtime)
